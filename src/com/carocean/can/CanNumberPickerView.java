 ...
 */
public class CanNumberPickerView extends View{
    ...
    private static final boolean DEFAULT_SHOW_DIVIDER = false;
    ...
    private Drawable mCurrentItemBg;
    // compatible for NumberPicker
    public interface OnValueChangeListener{
        void onValueChange(CanNumberPickerView picker, int oldVal, int newVal);
    }

    public interface OnValueChangeListenerRelativeToRaw{
        void onValueChangeRelativeToRaw(CanNumberPickerView picker, int oldPickedIndex, int newPickedIndex,
                                        String[] displayedValues);
    }

    public interface OnValueChangeListenerInScrolling{
        void onValueChangeInScrolling(CanNumberPickerView picker, int oldVal, int newVal);
    }
    ...
    public interface OnScrollListener {
        ...
        void onScrollStateChange(CanNumberPickerView view, int scrollState);
    }
    ...
    public CanNumberPickerView(Context context) {
        super(context);
        init(context);
    }
    public CanNumberPickerView(Context context, AttributeSet attrs) {
        super(context, attrs);
        initAttr(context, attrs);
        init(context);
    }
    public CanNumberPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
    ...
    private void initAttr(Context context, AttributeSet attrs){
        ...
        TypedArray a = context.obtainStyledAttributes(attrs, R.styleable.can_NumberPickerView);
        int n = a.getIndexCount();
        for (int i = 0; i < n; i++) {
            int attr = a.getIndex(i);
            if(attr == R.styleable.can_NumberPickerView_npv_ShowCount){
                mShowCount = a.getInt(attr, DEFAULT_SHOW_COUNT);
            }else if(attr == R.styleable.can_NumberPickerView_npv_DividerColor){
                mDividerColor = a.getColor(attr, DEFAULT_DIVIDER_COLOR);
            }else if(attr == R.styleable.can_NumberPickerView_npv_DividerHeight){
                mDividerHeight = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_HEIGHT);
            }else if(attr == R.styleable.can_NumberPickerView_npv_DividerMarginLeft){
                mDividerMarginL = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            }else if(attr == R.styleable.can_NumberPickerView_npv_DividerMarginRight){
                mDividerMarginR = a.getDimensionPixelSize(attr, DEFAULT_DIVIDER_MARGIN_HORIZONTAL);
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextArray){
                mDisplayedValues = convertCharSequenceArrayToStringArray(a.getTextArray(attr));
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextColorNormal){
                mTextColorNormal = a.getColor(attr, DEFAULT_TEXT_COLOR_NORMAL);
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextColorSelected){
                mTextColorSelected = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextColorHint){
                mTextColorHint = a.getColor(attr, DEFAULT_TEXT_COLOR_SELECTED);
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextSizeNormal){
                mTextSizeNormal = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_NORMAL_SP));
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextSizeSelected){
                mTextSizeSelected = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_SELECTED_SP));
            }else if(attr == R.styleable.can_NumberPickerView_npv_TextSizeHint){
                mTextSizeHint = a.getDimensionPixelSize(attr, sp2px(context, DEFAULT_TEXT_SIZE_HINT_SP));
            }else if(attr == R.styleable.can_NumberPickerView_npv_MinValue){
                mMinShowIndex = a.getInteger(attr, 0);
            }else if(attr == R.styleable.can_NumberPickerView_npv_MaxValue){
                mMaxShowIndex = a.getInteger(attr, 0);
            }else if(attr == R.styleable.can_NumberPickerView_npv_WrapSelectorWheel){
                mWrapSelectorWheel = a.getBoolean(attr, DEFAULT_WRAP_SELECTOR_WHEEL);
            }else if(attr == R.styleable.can_NumberPickerView_npv_ShowDivider){
                mShowDivider = a.getBoolean(attr, DEFAULT_SHOW_DIVIDER);
            }else if(attr == R.styleable.can_NumberPickerView_npv_HintText){
                mHintText = a.getString(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_AlternativeHint){
                mAlterHint = a.getString(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_EmptyItemHint){
                mEmptyItemHint = a.getString(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_MarginStartOfHint){
                mMarginStartOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_START_OF_HINT_DP));
            }else if(attr == R.styleable.can_NumberPickerView_npv_MarginEndOfHint){
                mMarginEndOfHint = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_MARGIN_END_OF_HINT_DP));
            }else if(attr == R.styleable.can_NumberPickerView_npv_ItemPaddingVertical){
                mItemPaddingVertical = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_V));
            }else if(attr == R.styleable.can_NumberPickerView_npv_ItemPaddingHorizontal){
                mItemPaddingHorizontal = a.getDimensionPixelSize(attr, dp2px(context, DEFAULT_ITEM_PADDING_DP_H));
            }else if(attr == R.styleable.can_NumberPickerView_npv_AlternativeTextArrayWithMeasureHint){
                mAlterTextArrayWithMeasureHint = a.getTextArray(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_AlternativeTextArrayWithoutMeasureHint){
                mAlterTextArrayWithoutMeasureHint = a.getTextArray(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_RespondChangeOnDetached){
                mRespondChangeOnDetach = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_ON_DETACH);
            }else if(attr == R.styleable.can_NumberPickerView_npv_RespondChangeInMainThread){
                mRespondChangeInMainThread = a.getBoolean(attr, DEFAULT_RESPOND_CHANGE_IN_MAIN_THREAD);
            }else if (attr == R.styleable.can_NumberPickerView_npv_TextEllipsize) {
                mTextEllipsize = a.getString(attr);
            }else if(attr == R.styleable.can_NumberPickerView_npv_CurrentItemBackground) {
            	mCurrentItemBg = a.getDrawable(attr);//context.getResources().getDrawable(a.getInt(attr, 0));
            }
        }
        a.recycle();
    }
    ...
    protected void onDetachedFromWindow() {
        ...
        // please initialize can_NumberPickerView's data every time setting up NumberPickerView,
        ...
        if(currPickedIndex != mPrevPickedIndex && mRespondChangeOnDetach){
            try {
                if (mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(CanNumberPickerView.this, mPrevPickedIndex + mMinValue, currPickedIndex + mMinValue);
                }
                if (mOnValueChangeListenerRaw != null) {
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(CanNumberPickerView.this, mPrevPickedIndex, currPickedIndex, mDisplayedValues);
                }
            }catch (Exception e){
                e.printStackTrace();
            }
    ...
    private void respondPickedValueChanged(int oldVal, int newVal, Object respondChange){
        ...
        if(oldVal != newVal){
            if(respondChange == null || !(respondChange instanceof Boolean) || (Boolean) respondChange) {
                if(mOnValueChangeListener != null) {
                    mOnValueChangeListener.onValueChange(CanNumberPickerView.this, oldVal + mMinValue, newVal + mMinValue);
                }
                if(mOnValueChangeListenerRaw != null){
                    mOnValueChangeListenerRaw.onValueChangeRelativeToRaw(CanNumberPickerView.this, oldVal, newVal, mDisplayedValues);
                }
            }
        }
    ...
    private int findIndexByValue(int value) {
    	...
    	for(int i = 0; i < mDisplayedValues.length; i ++) {
    		if(mDisplayedValues[i].equals(String.valueOf(value))) {
    			index = i;
    			break;
    		}
    	}
    ...
    private int mCurrDrawFirstItemY = 10;
    ...
    protected void onDraw(Canvas canvas) {
        super.onDraw(canvas);
        drawCurrentItemBackGround(canvas);
        drawContent(canvas);
        drawLine(canvas);
        drawHint(canvas);
    }
    ...
    private void drawCurrentItemBackGround(Canvas canvas){
    	if(null != mCurrentItemBg) {
    		...
    		mCurrentItemBg.setBounds(left, top, right, bottom);
    		mCurrentItemBg.draw(canvas);
    	}
    }
