...
final class Binding implements View.OnAttachStateChangeListener {
	...
	public void onViewAttachedToWindow(View v) {
		if (v != view) {
			throw new AssertionError("Binding for view " + view + " notified of attachment of different view " + v);
		}
		if (coordinator.isAttached()) {
			throw new IllegalStateException("Coordinator " + coordinator + " is already attached");
		}
	...
	public void onViewDetachedFromWindow(View v) {
		if (v != view) {
			throw new AssertionError("Binding for view " + view + " notified of detachment of different view " + v);
		}
		if (!coordinator.isAttached()) {
			// Android tries not to make reentrant calls, but doesn't always
			// succeed.
