package com.carocean.launcher.wheelview;
...
public class CycleWheelView extends ListView {
	...
	private void init() {
		...
		mItemLabelTvId2 = R.id.tv_label_item_wheel2;
	...
	private void resetItems(int firstPosition, int position, int offset) {
		for (int i = position - offset - 1; i < position + offset + 1; i++) {
			...
			TextView labelTv2 = (TextView) itemView.findViewById(mItemLabelTvId2);
			if (position == i) {
				labelTv.setTextColor(mLabelSelectColor);
				labelTv2.setTextColor(mLabelSelectColor);
				itemView.setAlpha(1f);
			} else {
				labelTv.setTextColor(mLabelColor);
				labelTv2.setTextColor(mLabelColor);
	 ...
	 *            刻度TextView的资源Id
	 */
	public void setWheelItemLayout(int itemResId, int labelTvId,int labelTvId2) {
		mItemLayoutId = itemResId;
		mItemLabelTvId = labelTvId;
		mItemLabelTvId2 = labelTvId2;
		mAdapter = new CycleWheelViewAdapter();
		mAdapter.setData(mLabels);
		setAdapter(mAdapter);
		initView();
	...
	private void initView() {
		...
		Drawable backgroud = new Drawable() {
			...
			public void draw(Canvas canvas) {
				...
				canvas.drawRect(0, mItemHeight * (mWheelSize / 2 + 1), viewWidth, mItemHeight * (mWheelSize),
						solidPaint);
				canvas.drawRect(0, mItemHeight * (mWheelSize / 2), viewWidth, mItemHeight * (mWheelSize / 2 + 1),
						seletedSolidPaint);
				canvas.drawLine(0, mItemHeight * (mWheelSize / 2), viewWidth, mItemHeight * (mWheelSize / 2),
						dividerPaint);
				canvas.drawLine(0, mItemHeight * (mWheelSize / 2 + 1), viewWidth, mItemHeight * (mWheelSize / 2 + 1),
						dividerPaint);
			}
	...
	public class CycleWheelViewAdapter extends BaseAdapter {
		...
		public View getView(int position, View convertView, ViewGroup parent) {
			...
			TextView textView2 = (TextView) convertView.findViewById(mItemLabelTvId2);
			if (position < mWheelSize / 2 || (!cylceEnable && position >= mData.size() + mWheelSize / 2)) {
				textView.setText("");
				textView2.setText("");
				convertView.setVisibility(View.INVISIBLE);
			} else {
				textView.setText(mData.get((position - mWheelSize / 2) % mData.size())[0]);
				textView2.setText(mData.get((position - mWheelSize / 2) % mData.size())[1]);
