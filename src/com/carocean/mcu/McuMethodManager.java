package com.carocean.mcu;

import static android.mcu.McuExternalConstant.MCU_SYSTEM_PARAM_SLEEP_INDEX;
...
import static android.mcu.McuExternalConstant.T_QB_POWER_OFF;
import static android.mcu.McuExternalConstant.T_SYS_RESTART;
...
public class McuMethodManager {
	...
	public void setMcuSystemParam(SystemParam arg0) {
		...
		videoParma[5] = (byte) (arg0.backlight * 100 / 255);
		...
		sLog.e("setMcuSystemParam - videoParma: " + sb.toString());
...
/*		byte[] parma = new byte[7];
	...
	public void setCantype() {
		...
		// ArrayList<String> list_can_type =
		// XmlParse.getStringArray(XmlParse.support_car_type);
	...
	public void setMcuParam_radio12v(boolean value) {
		byte[] Parma = new byte[3];
		Parma[0] = 0x0a;
		if (value) {
			Parma[1] = 1;
	...
	public void setMcuParam_externAmp(boolean value) {
		byte[] Parma = new byte[3];
		Parma[0] = 0x0b;
		if (value) {
			Parma[1] = 1;
	...
	public void setMcuParam_VcommVoltage(int value) {
		byte[] Parma = new byte[3];
		Parma[0] = 0x0c;
		Parma[1] = (byte)(value&0xff);
	...
	public void setMcuParam_backcar() {
		...
		boolean brake_enable = SystemProperties.getBoolean(PersistUtils.PERSYS_BRAKE_ENABLE, false);
		boolean backcar_enable = SystemProperties.getBoolean(PersistUtils.PERSYS_BACKCAR_ENABLE, true);
		boolean backcar_mute = SystemProperties.getBoolean(PersistUtils.PERSYS_BACKCAR_MUTE, false);
		boolean backcar_guidelines = SystemProperties.getBoolean(PersistUtils.PERSYS_BACKCAR_TRACE, false);
		boolean backcar_radar = SystemProperties.getBoolean(PersistUtils.PERSYS_BACKCAR_RADAR, false);
		boolean antenna_ctrl_enable = SystemProperties.getBoolean(PersistUtils.PERSYS_ANTENNA_CTRL, false);
		boolean power_amplifier_ctrl_enable = SystemProperties.getBoolean(PersistUtils.PERSYS_POWER_AMPLIFIER_CTRL, false);
//		int videoport_front_camera = SystemProperties.getInt(Tag.PERSYS_VIDEO_PORT_FRONT_CAMERA, XmlParse.videoport_front_camera);
...
/*	public void setMcuParam_sleepTime() {
	...
	static boolean sIsFirstStart = false;

	public void setMcuParam_qicaideng() {
		byte[] data = new byte[4];
		int len = 4;
		if (!sIsFirstStart) {
			sIsFirstStart = SystemProperties.getBoolean(Tag.PERSYS_IS_FIRST_START, false);
			if (!sIsFirstStart) {
				sIsFirstStart = true;
				SystemProperties.set(Tag.PERSYS_IS_FIRST_START, "true");
				SystemProperties.set(Tag.PERSYS_BACK_LIGHT_R, XmlParse.light_r + "");
				SystemProperties.set(Tag.PERSYS_BACK_LIGHT_G, XmlParse.light_g + "");
				SystemProperties.set(Tag.PERSYS_BACK_LIGHT_B, XmlParse.light_b + "");
			}
		}
		data[0] = (byte) XmlParse.light_mode;
		data[1] = (byte) SystemProperties.getInt(Tag.PERSYS_BACK_LIGHT_R, XmlParse.light_r);
		data[2] = (byte) SystemProperties.getInt(Tag.PERSYS_BACK_LIGHT_G, XmlParse.light_g);
		data[3] = (byte) SystemProperties.getInt(Tag.PERSYS_BACK_LIGHT_B, XmlParse.light_b);
