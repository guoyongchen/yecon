...
public class HanziToPinyin {
    private static final String TAG = "HanziToPinyin-media";
    ...
    private String getLetter(char character) {
    	String str = "";
        Token token = new Token();
        final String letter = Character.toString(character);
        token.source = letter;
     ...
     * Token. If these is no China collator, the empty token array is returned.
     */
    public String getFirstLetter(final String input) {
    	String string = "";
        if (!mHasChinaCollator || TextUtils.isEmpty(input)) {
            // return empty tokens.
            return string;
