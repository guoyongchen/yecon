...
public class CharacterParser {
	...
	private static StringBuilder pinyin[] = new StringBuilder[4];
	...
	public static String letterstodialnum(String letters){
		String res = "";
			for (int i = 0; i < letters.length(); i++) {
				for (Entry<String, String> entry : map.entrySet()) {
					if (entry.getValue().contains(String.valueOf(letters.charAt(i)))) {
						res += entry.getKey();
						break;
					}
				}
	...
	//string[0] full pin yin  string[1]head pin yin
	public static StringBuilder[] getSelling(String chs) {
		String key, value;		
		buffer.setLength(0);//全拼
		buffer1.setLength(0);//首拼
		buffer2.setLength(0);//全摒转拨号数字
		buffer3.setLength(0);//首拼转拨号数字
		for (int i = 0; i < chs.length(); i++) {
			...
			buffer.append(value);
			buffer2.append(" ").append(letterstodialnum(value));
			if(value.length()>1){
				value = value.substring(0, 1);
			}
			buffer1.append(value);
			buffer3.append(letterstodialnum(value));
		}
		pinyin[0] = buffer;
		pinyin[1] = buffer1;
		pinyin[2] = buffer2;
	...
	public static String getSellingFull(String chs) {
		String key, value;
		buffer.setLength(0);
		for (int i = 0; i < chs.length(); i++) {
			key = chs.substring(i, i + 1);
