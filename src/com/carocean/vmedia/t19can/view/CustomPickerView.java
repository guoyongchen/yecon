...
/**
 * 滚轮视图，可设置是否循环模式，实现OnScrollChangedListener接口以监听滚轮变化 Created by huzn on
 * 2016/10/27.
 */
public class CustomPickerView extends View {
	...
	public CustomPickerView(Context context, AttributeSet attrs, int defStyleAttr) {
		...
		TypedArray a = context.getTheme().obtainStyledAttributes(attrs, R.styleable.CustomPickerView, defStyleAttr, 0);
		textSize = a.getDimensionPixelSize(R.styleable.CustomPickerView_epvTextSize,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_SP, 16, getResources().getDisplayMetrics()));
		textColor = a.getColor(R.styleable.CustomPickerView_epvTextColor, Color.BLACK);
		textPadding = a.getDimensionPixelSize(R.styleable.CustomPickerView_epvTextPadding,
				(int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 20, getResources().getDisplayMetrics()));
		textMaxScale = a.getFloat(R.styleable.CustomPickerView_epvTextMaxScale, 2.0f);
		textMinAlpha = a.getFloat(R.styleable.CustomPickerView_epvTextMinAlpha, 0.4f);
		isRecycleMode = a.getBoolean(R.styleable.CustomPickerView_epvRecycleMode, true);
		maxShowNum = a.getInteger(R.styleable.CustomPickerView_epvMaxShowNum, 3);
		a.recycle();
	...
	protected void onDraw(Canvas canvas) {
		if (null != dataList && dataList.size() > 0) {
			canvas.clipRect(cx - contentWidth / 2, cy - contentHeight / 2, cx + contentWidth / 2,
					cy + contentHeight / 2);
			...
			for (int i = -half; i <= half; i++) {
				...
				if (index >= 0 && index < size) {
					...
					// 根据textMaxScale，计算出tempScale值，即实际text应该放大的倍数，范围
					// 1~textMaxScale
